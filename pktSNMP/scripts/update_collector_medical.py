#!/usr/bin/env python3
"""
update_collector_medical.py — Redirect medical otelcol SNMP metrics to pktSNMP.

Collector host : 172.23.80.11 (ec2-user, VyneCorpNetInfra.pem)
pktSNMP host   : 172.23.80.5  (ec2-user, VyneCorpNetInfra.pem)
Collector id   : 2
SNMP pipelines : metrics/switch, metrics/firewall
...
COLLECTOR_HOST  = "172.23.80.11"
COLLECTOR_USER  = "ec2-user"
COLLECTOR_KEY_PATH = r"C:\Users\robert.barnett\.ssh\VyneCorpNetInfra.pem"
...
SNMP_PIPELINES  = ["metrics/switch", "metrics/firewall"]
# ────────────────────────────────────────────────────────────────────────────────
...
def main():
    ...
    ok(f"Token generated (shown once at end)")
    ...
    try:
        sql = (
            f"UPDATE collectors SET api_token='{token}', "
            f"updated_at=datetime('now') WHERE id={COLLECTOR_ID};"
        )
        out, err, code = run(o2_client, f"sqlite3 {DB_PATH} \"{sql}\"")
    ...
    try:
        ...
        run(col_client, f"cp {CONFIG_PATH} {BACKUP_PATH}")
        backup_done = True
        ok("Backup created")
        ...
        validate_bin = "/mnt/software/otel/otelcol" if out.strip() == "found" else "otelcol"
        ok(f"Using binary: {validate_bin}")
        ...
        if code != 0:
            ...
            run(col_client, f"cp {BACKUP_PATH} {CONFIG_PATH}")
            ok("Backup restored")
            sys.exit(1)
    ...
    print("\n" + "=" * 60)
    print("  Medical collector update COMPLETE")
