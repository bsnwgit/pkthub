...
In pktHub this sits beside the routes rather than under app/api/, since pktHub
keeps its route modules flat in app/ — but the package itself stays at this
path so it remains directly comparable with the sibling copies.
"""
...
# to resonance ("pkthub-alice"), so its logs show both who and which app. A
...
# running on it, which is why the login page is the only default. The public
# NOC display is pktHub's own addition: /display/:token is deliberately
# unauthenticated and goes on a wall, so an assistant must never appear there.
DEFAULT_EXCLUDE_PATHS = ["/login", "/display"]
