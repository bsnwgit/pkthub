...
pkthub's own log records are written to the local app_logs table for the
...
log = logging.getLogger("pkthub.log_forward")
...
class SyslogForwardHandler(logging.Handler):
    ...
    def __init__(
        ...
        self._thread = threading.Thread(
            target=self._run, name="pkthub-log-forward", daemon=True
        )
        self._thread.start()
    ...
    def emit(self, record: logging.LogRecord) -> None:
        ...
        if record.name.startswith("pkthub.log_forward"):
            return
        try:
...
def send_test_message(host: str, port: int, protocol: str, app_name: str = "pkthub") -> dict:
    ...
    try:
        rec = logging.LogRecord(
            name="pkthub.test", level=logging.INFO, pathname=__file__, lineno=0,
            msg="pkthub log forwarding test message", args=(), exc_info=None,
        )
        h.emit(rec)
