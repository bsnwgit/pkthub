...
for use by future IP/reputation lookup features.
...
# Providers this app knows how to use. ip-api.com deliberately has no entry
# here — it needs no API key, so there's nothing for a user to store.
...
# (see app/ip_info.py's _fetch_mxtoolbox) — the rest are domain/email
...
async def set_provider_enabled(
    ...
    await db.execute(
        ...
        (current_user["username"], provider, int(body.enabled)),
    )
    await db.commit()
    async with db.execute(
...
async def set_ipapi_is_free_tier(
    body: FreeTierIn,
    current_user: dict = Depends(get_current_user),
    db: aiosqlite.Connection = Depends(get_db),
):
    await db.execute(
        ...
        (current_user["username"], int(body.free_tier)),
    )
    await db.commit()
    async with db.execute(
...
async def set_ipinfo_fields(
    ...
    await db.execute(
        ...
        (current_user["username"], json.dumps(body.enabled_fields)),
    )
    await db.commit()
    async with db.execute(
...
async def set_mxtoolbox_fields(
    body: FieldsIn,
    current_user: dict = Depends(get_current_user),
    db: aiosqlite.Connection = Depends(get_db),
):
...
async def set_api_key(
    ...
    if key:
        ...
        # rather than deleting it outright, so any other stored preference
        # for this provider (enabled_fields/free_tier) survives
        # clearing/re-saving a blank key.
        await db.execute(
            """INSERT INTO user_api_keys (username, provider, api_key, updated_at)
               VALUES (?, ?, '', datetime('now'))
               ON CONFLICT (username, provider)
