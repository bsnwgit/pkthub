...
(see app/user_api_keys.py). Private/loopback/link-local addresses are
...
async def _get_user_key(db: aiosqlite.Connection, username: str, provider: str) -> str:
...
async def _get_ipapi_is_enabled_fields(db: aiosqlite.Connection, username: str) -> list[str] | None:
    async with db.execute(
...
async def _get_ipapi_is_free_tier(db: aiosqlite.Connection, username: str) -> bool:
    async with db.execute(
...
async def _get_mxtoolbox_enabled_fields(db: aiosqlite.Connection, username: str) -> list[str] | None:
    async with db.execute(
...
async def get_ip_info(
    ...
    ipinfo_key = await _get_user_key(db, current_user["username"], "ipinfo")
    ipapi_is_key = await _get_user_key(db, current_user["username"], "ipapi_is")
    abuseipdb_key = await _get_user_key(db, current_user["username"], "abuseipdb")
    mxtoolbox_key = await _get_user_key(db, current_user["username"], "mxtoolbox")
    result.ipinfo_enabled_fields = await _get_ipinfo_enabled_fields(db, current_user["username"])
    result.ipapi_is_enabled_fields = await _get_ipapi_is_enabled_fields(db, current_user["username"])
    result.mxtoolbox_enabled_fields = await _get_mxtoolbox_enabled_fields(db, current_user["username"])
    ipapi_is_free_tier = await _get_ipapi_is_free_tier(db, current_user["username"])
