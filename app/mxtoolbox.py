...
app/user_api_keys.py). Covers every command MXToolbox's API supports, not
...
@router.post("/lookup")
async def mxtoolbox_lookup(
    body: MxToolboxLookupRequest,
    ...
    key = await _get_user_key(db, current_user["username"], "mxtoolbox")
    if not key:
