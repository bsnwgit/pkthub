#!/bin/bash
# pktHub uninstall script — Ubuntu Server 22.04/24.04 LTS
...
# credential encryption key), pkthub.db and its -wal/-shm, logs, backups and
...
# WorkingDirectory, then $PKTHUB_INSTALL_DIR, then /opt/pkthub.

set -euo pipefail

APP="pkthub"
DISPLAY="pktHub"
UNIT_PATH="/etc/systemd/system/pkthub.service"
...
# migration — plain *.db does not match "pkthub.db.pre-upgrade".
...
if [ "$PURGE" -eq 1 ]; then
    ...
    for entry in config.yaml pkthub.db logs backups ssl; do
        if [ -e "$INSTALL_DIR/$entry" ]; then
            echo "    $entry"
        fi
    done
