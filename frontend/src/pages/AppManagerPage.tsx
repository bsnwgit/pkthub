...
export default function AppManagerPage() {
  ...
  const toggleMode = async (app: any) => {
    const newMode = app.mode === 'managed' ? 'observe' : 'managed'
    if (newMode === 'managed' && !confirm(`Switch ${app.name} to MANAGED mode?\n\nThis blocks direct access — all traffic must go through pktSuite.`)) return
    try {
    ...
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-400 mt-0.5">Register, monitor, and control pktXXXX apps</p>
        </div>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <div className="flex items-center gap-2">
                      ...
                      <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${app.mode === 'managed' ? 'bg-orange-900/30 text-orange-300' : 'bg-blue-900/20 text-blue-300'}`}>
                  ...
                  {isAdmin && (
                    <>
                      <button
                        ...
                        title={app.mode === 'managed' ? 'Switch to Observe' : 'Switch to Managed'}
                        className="p-1.5 text-gray-400 hover:text-white rounded-lg hover:bg-gray-700 transition-colors"
                      >
                        {app.mode === 'managed' ? <Eye size={14} /> : <ShieldCheck size={14} />}
                      </button>
                      <button
