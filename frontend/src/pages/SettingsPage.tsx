...
const APP_SECTION_LABEL = 'pktHub'
...
export default function SettingsPage() {
  ...
  const [section, setSection] = useState<SectionId>(
    sectionOfTab(initialTab && TABS.some(t => t.id === initialTab) ? initialTab : 'general')
  )
  ...
  // This settings store is string-valued, so the list round-trips as a JSON string.
  ...
  const localProviders: LocalProvider[] = (() => {
    try {
      const raw = settings['ai_local_providers']
      const parsed = raw ? JSON.parse(raw) : []
      return Array.isArray(parsed) ? parsed : []
    } catch { return [] }
  })()
  const updateLocalProvider = (idx: number, patch: Partial<LocalProvider>) => {
    set('ai_local_providers', JSON.stringify(localProviders.map((p, i) => i === idx ? { ...p, ...patch } : p)))
  }
  const addLocalProvider = () => {
    const id = typeof crypto !== 'undefined' && crypto.randomUUID ? crypto.randomUUID() : `local-${Date.now()}`
    set('ai_local_providers', JSON.stringify([...localProviders, { id, name: '', base_url: '', model: '', api_key: '', enabled: false }]))
  }
  const removeLocalProvider = (idx: number) => {
    set('ai_local_providers', JSON.stringify(localProviders.filter((_, i) => i !== idx)))
  }
            ...
            {securityTab === 'ai' && (
              <Section title="AI Assistant" onSave={aiAssistantSave.save} saving={aiAssistantSave.saving} saved={aiAssistantSave.saved} error={aiAssistantSave.error}
                help={{
                  ...
                  content: <>
                    <p>Providers are tried top to bottom — <span className="text-gray-300 font-medium">local / self-hosted providers first</span>, since they keep your data on hardware you control, then cloud providers as a fallback. Turn on as many as you like; the first enabled provider with valid config answers each question in the chat panel.</p>
                    <p className="mt-2"><span className="text-gray-300 font-medium">Ollama</span> and other local endpoints run entirely on your own hardware — nothing is sent off-network. <span className="text-gray-300 font-medium">Anthropic</span> is a paid, cloud-hosted API and needs its own key from console.anthropic.com, separate from a Claude Enterprise seat.</p>
                    <p className="mt-2">Unlike the sibling apps' assistants, pktHub's is scoped to <span className="text-gray-300 font-medium">its own data</span> — the app registry, NOC dashboards, and audit log — not any individual pktApp's telemetry.</p>
                  </>,
                }}
              >
