...
function AppRegistrySection({ settings, set, save }: {
  ...
  const blankForm = { name: '', base_url: '', description: '', return_url: '' }
  const [form, setForm] = useState({ ...blankForm })
  ...
  const register = async () => {
    ...
    try {
      ...
      setForm({ name: '', base_url: '', description: '', return_url: '' })
  ...
  const toggleMode = async (app: any) => {
    ...
    try { await api.setAppStatus(app.id, newMode); loadApps() }
    catch (e: any) { alert(e.message) }
  }
        ...
        {showForm && isAdmin && (
          <div className="px-5 py-4 border-b border-gray-800 space-y-3">
            ...
            <div className="flex gap-2">
              ...
              <button onClick={() => { setShowForm(false); setEditingAppId(null); setForm({ name: '', base_url: '', description: '', return_url: '' }) }} className="px-4 py-2 rounded-lg text-sm text-gray-400 hover:text-gray-200">Cancel</button>
            </div>
          </div>
        )}
              ...
              <div key={app.id} className="flex items-center gap-3 px-5 py-3" style={{ borderLeft: `3px solid ${color}30` }}>
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    ...
                    <span className={`text-xs px-1.5 py-0.5 rounded font-medium ${app.status === 'managed' ? 'bg-orange-900/30 text-orange-300' : 'bg-blue-900/20 text-blue-300'}`}>
                ...
                <div className="flex items-center gap-1 shrink-0">
                  ...
                  {isAdmin && (
                    <>
                      <button onClick={() => toggleMode(app)} title={app.status === 'managed' ? 'Switch to Observe' : 'Switch to Managed'}
                        className="p-1.5 text-gray-500 hover:text-white rounded transition-colors">
                        {app.status === 'managed' ? <Eye size={13} /> : <ShieldCheck size={13} />}
                      </button>
                      ...
                      <button onClick={() => { setEditingAppId(app.id); setForm({ name: app.name, base_url: app.base_url, description: app.description || '', return_url: app.return_url || '' }); setRegError(''); setShowForm(true) }} title="Edit app"
